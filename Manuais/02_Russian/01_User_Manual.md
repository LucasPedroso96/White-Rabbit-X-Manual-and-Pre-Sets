# White Rabbit X — Полное руководство
...
MTF — слой согласования MACD; MA, ADX, ATR volatility и news независимы. Каждый новый фильтр уменьшает выборку.
...
Исполнение зависит от spread, margin, volume step, tick size и ограничений stops/freeze брокера. Проверяйте результат в Journal.
...
Stop, ATR take, breakeven, ATR trailing и reversal exit независимы. ReversalExitUseEntryFilters управляет применением фильтров к выходу.

## Риск и восстановление

Выберите один PositionSizeMode. Percentage и Fixed-R требуют stop. Grid, Martingale и D'Alembert — исследовательские режимы восстановления.

## Расписание, интерфейс и WFO

Время — сервер брокера; поддерживаются ночные интервалы и отдельные Saturday/Sunday. WFO задаёт IS/OOS и критерий.
...
## Полный список inputs

Таблица создана в порядке исходника. Строки-разделители сохраняются для совместимости .set.
