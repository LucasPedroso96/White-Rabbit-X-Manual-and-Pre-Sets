# White Rabbit X — Eksiksiz Kullanım Kılavuzu
...
MTF üst zaman MACD hizalamasıdır; MA, ADX, ATR volatility ve news bağımsızdır.
...
## Yürütme ve pozisyon yönetimi

Spread, margin, volume step, tick size ve stops/freeze sınırları emri etkiler. Journal üzerinden doğrulayın.
...
Stop, ATR take, breakeven, ATR trailing ve reversal exit ayrıdır. ReversalExitUseEntryFilters çıkış filtresini yönetir.

## Risk ve toparlanma

Tek PositionSizeMode seçin. Percentage/Fixed-R stop ister. Grid, Martingale ve D'Alembert araştırma modlarıdır.

## Zaman, arayüz ve WFO

Broker sunucu zamanı, gece aralığı ve Saturday/Sunday desteklenir. WFO IS/OOS tanımlar.
...
## Tam inputs referansı

Tablo kaynak sırasındadır. Ayırıcı dizeler .set şemasının parçasıdır.
