...
const i18n = {
  en: {
    ...
    'setup.economic-mode-label': 'Enable Hedge (buy + sell together)',
    'setup.economic-mode-desc': 'Runs buy and sell combined in a single account/certificate instead of two separate runs -- cuts calibration work in half per system. Hedging becomes a searchable parameter (the genetic search decides whether keeping both sides open at once performs better). Skips no safety gate (survival, long holdout, Monte Carlo) -- a bad combined result is rejected like any other candidate.',
  ...
  pt: {
    ...
    'setup.economic-mode-label': 'Ativar Hedge (compra e venda juntos)',
    'setup.economic-mode-desc': 'Roda compra e venda combinados numa única conta/certificado em vez de duas corridas separadas -- reduz o trabalho de calibração pela metade por sistema. O Hedge vira um parâmetro pesquisável (o genético decide se manter os dois lados abertos ao mesmo tempo rende mais). Não pula nenhum gate de segurança (sobrevivência, holdout longo, Monte Carlo) -- um resultado combinado ruim é reprovado como qualquer outro candidato.',
  ...
  es: {
    ...
    'setup.economic-mode-label': 'Activar Hedge (compra y venta juntas)',
    'setup.economic-mode-desc': 'Ejecuta compra y venta combinadas en una sola cuenta/certificado en vez de dos corridas separadas -- reduce a la mitad el trabajo de calibración por sistema. El Hedge se vuelve un parámetro buscable (el genético decide si mantener ambos lados abiertos a la vez rinde más). No omite ningún filtro de seguridad (supervivencia, holdout largo, Monte Carlo) -- un resultado combinado malo se rechaza como cualquier otro candidato.',
  ...
  de: {
    ...
    'setup.economic-mode-label': 'Hedge aktivieren (Kauf + Verkauf zusammen)',
    'setup.economic-mode-desc': 'Führt Kauf und Verkauf kombiniert auf einem einzigen Konto/Zertifikat aus statt zwei getrennter Läufe -- halbiert den Kalibrierungsaufwand pro System. Hedge wird zu einem durchsuchbaren Parameter (die genetische Suche entscheidet, ob beide Seiten gleichzeitig offen zu halten mehr bringt). Kein Sicherheits-Gate wird übersprungen (Überleben, langes Holdout, Monte Carlo) -- ein schlechtes kombiniertes Ergebnis wird wie jeder andere Kandidat abgelehnt.',
  ...
  fr: {
    ...
    'setup.economic-mode-label': 'Activer le Hedge (achat + vente ensemble)',
    'setup.economic-mode-desc': "Exécute achat et vente combinés sur un seul compte/certificat au lieu de deux passages séparés -- réduit de moitié le travail de calibration par système. Le Hedge devient un paramètre optimisable (la recherche génétique décide si garder les deux côtés ouverts en même temps rapporte plus). Aucun filtre de sécurité n'est contourné (survie, holdout long, Monte Carlo) -- un mauvais résultat combiné est rejeté comme tout autre candidat.",
  ...
  it: {
    ...
    'setup.economic-mode-label': 'Attiva Hedge (acquisto + vendita insieme)',
    'setup.economic-mode-desc': "Esegue acquisto e vendita combinati in un unico conto/certificato invece di due run separate -- dimezza il lavoro di calibrazione per sistema. L'Hedge diventa un parametro ricercabile (la ricerca genetica decide se tenere aperti entrambi i lati contemporaneamente rende di più). Nessun gate di sicurezza viene saltato (sopravvivenza, holdout lungo, Monte Carlo) -- un risultato combinato negativo viene respinto come qualsiasi altro candidato.",
  ...
  ru: {
    ...
    'setup.economic-mode-label': 'Включить хедж (покупка и продажа вместе)',
    'setup.economic-mode-desc': 'Запускает покупку и продажу вместе на одном счёте/сертификате вместо двух отдельных прогонов -- вдвое сокращает работу по калибровке для каждой системы. Хедж становится параметром для поиска (генетический алгоритм сам решает, выгоднее ли держать обе стороны открытыми одновременно). Ни один защитный фильтр не пропускается (выживаемость, длинный holdout, Монте-Карло) -- плохой совмещённый результат отклоняется, как и любой другой кандидат.',
  ...
  zh: {
    ...
    'setup.economic-mode-label': '启用对冲（买卖同时进行）',
    'setup.economic-mode-desc': '在同一账户/证书中同时运行买入和卖出，而不是分两次独立运行——每个系统的校准工作量减半。对冲变成一个可搜索的参数（遗传算法自行决定同时保持两个方向是否表现更好）。不会跳过任何安全关卡（生存性、长期留存、蒙特卡洛)——组合结果不好的话，会像其他候选项一样被拒绝。',
  ...
  ja: {
    ...
    'setup.economic-mode-label': 'ヘッジを有効にする（買いと売りを同時に）',
    'setup.economic-mode-desc': '買いと売りを別々に2回実行する代わりに、同じ口座・証明書内でまとめて実行します — システムごとのキャリブレーション作業が半分になります。ヘッジは探索可能なパラメータになります（両サイドを同時に開いておく方が有利かどうかは遺伝的アルゴリズムが判断します）。安全ゲート（生存性、長期ホールドアウト、モンテカルロ）は一切スキップされません — 組み合わせの結果が悪ければ、他の候補と同様に却下されます。',
  ...
  ko: {
    ...
    'setup.economic-mode-label': '헤지 활성화 (매수+매도 동시 진행)',
    'setup.economic-mode-desc': '두 번의 별도 실행 대신 매수와 매도를 하나의 계정/인증서에서 함께 실행합니다 -- 시스템당 캘리브레이션 작업이 절반으로 줄어듭니다. 헤지는 탐색 가능한 매개변수가 됩니다(두 방향을 동시에 열어두는 것이 더 나은지는 유전 알고리즘이 결정합니다). 어떤 안전 게이트도 건너뛰지 않습니다(생존성, 장기 홀드아웃, 몬테카를로) -- 결합 결과가 나쁘면 다른 후보와 마찬가지로 거부됩니다.',
  ...
  tr: {
    ...
    'setup.economic-mode-label': "Hedge'i etkinleştir (alış + satış birlikte)",
    'setup.economic-mode-desc': 'Alış ve satışı iki ayrı çalıştırma yerine tek bir hesapta/sertifikada birlikte çalıştırır -- sistem başına kalibrasyon işini yarıya indirir. Hedge aranabilir bir parametre haline gelir (her iki tarafı aynı anda açık tutmanın daha iyi sonuç verip vermediğine genetik arama karar verir). Hiçbir güvenlik kapısı atlanmaz (hayatta kalma, uzun holdout, Monte Carlo) -- kötü bir birleşik sonuç diğer adaylar gibi reddedilir.',
