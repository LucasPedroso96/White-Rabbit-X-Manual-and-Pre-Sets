...
8. Проверяйте победителя на реальных тиках: решают расхождение с OHLC и out-of-sample-ретенция, а не прибыль in-sample. Отклоняйте кандидата, чей результат на реальных тиках расходится с результатом на OHLC более чем на 30% — оба должны совпадать по форме результата, а не только по знаку.
8a. Проверяйте выжившего по Monte Carlo: пересэмплируйте последовательность сделок (в R-кратных величинах, не в валюте) и отклоняйте, если просадка на 95-м перцентиле превышает вдвое наблюдаемую просадку, или если пересэмплированная вероятность разорения превышает 5%. Набор, который выглядит стабильным только из-за конкретного порядка сделок, не является стабильным.
8b. Для шести систем Fixed-R (`01`–`06`) требуйте положительное R-ожидание вне выборки (out-of-sample). Набор, который вышел в ноль или потерял R вне выборки, не продвигается, независимо от результата in-sample.
...
## Формулы: что они оптимизируют и что отражает результат

`selectedFormula` определяет, что OnTester возвращает генетическому оптимизатору — единственное число, по которому ранжируется прогон. Это не то же самое, что «в чём отчитывается итоговый набор». Контур использует разные формулы для разных задач: более ранние фазы отдают предпочтение формулам, которые вознаграждают широкий, хорошо заполненный результат (чтобы у генетического поиска был градиент для подъёма), а не узкой формуле, которая просто высоко оценивает один конкретный путь.

Для шести систем Fixed-R итоговый отчёт поставляемого набора использует **SomaR** (сумму результатов сделок в R-кратных величинах): после того как кандидат уже прошёл ретенцию, дивергенцию, Monte Carlo и указанный выше гейт R-ожидания, SomaR — это то, что выражает результат в той же единице, которую это руководство использует для сравнения символов и систем, — в R, а не в валюте. Она не решает, кто победитель; она отражает результат уже победившего набора в сопоставимой единице.

## Autobot и Historical Tool Manager

Эта библиотека поставляется предварительно провалидированной, но описанный выше контур — не чёрный ящик: он опубликован как **Autobot** в том же репозитории, где находится это руководство (`Autobot/`), — это реальный код, который выполняет все шаги данного руководства. Прочитайте его, чтобы увидеть точно, как набор заслужил свой статус, или запустите его сами против своего брокера, списка символов или диапазона дат.

Этап подтверждения на реальных тиках зависит от наличия реальных тиковых данных для сверки. **Historical Tool Manager** (MQL5 Market: https://www.mql5.com/pt/market/product/188711) импортирует глубокую историю тиков и M1 в MT5 в виде Custom Symbols для инструментов, история которых у самого брокера не покрывает достаточно далёкий период, — это полезно как при использовании Autobot, так и просто для расширения истории при ручном тестировании.
