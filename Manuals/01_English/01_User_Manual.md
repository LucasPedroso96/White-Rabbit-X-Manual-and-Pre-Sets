# White Rabbit X — Complete User Manual
...
Compile or install the matching EX5, copy presets into the terminal Tester profile, select the correct broker symbol, load one set in Strategy Tester Inputs and verify every validation message.
...
MTF alignment is a MACD higher-timeframe alignment layer. MA, ADX, ATR-volatility and news are independent filters. Enabling more filters narrows the sample; optimize stacks as hypotheses, not as a promise of higher quality.
...
Spread, margin, broker volume steps, tick size, stops level and freeze level affect execution. Protective prices are part of the entry request when enabled. Closing requests can require a later retry, so inspect the Journal and never infer execution only from a visual signal.
...
Stop loss, organic ATR take profit, breakeven, ATR trailing and reversal exit are separate controls. Indicator reversal exits use the opposite raw signal by default; enable ReversalExitUseEntryFilters only when the entry filters must also confirm the exit.

## Risk and recovery

Choose one PositionSizeMode. Percentage and Fixed-R require a valid stop. Grid, Martingale and D'Alembert are recovery research modes, not independent position-size modes. Exposure limits, daily loss, equity drawdown and minimum free margin are hard safety constraints.

## Schedule, interface and WFO

Trading hours use broker server time and may cross midnight. Trading days include explicit Saturday and Sunday controls. The live/visual dashboard is optional. WFO inputs delimit the in-sample window, out-of-sample step and optimization criterion.
...
## Discovered set library

Every .set file is a research or optimization hypothesis. Counts below come from the current filesystem and manifest; they are not embedded in this document template. Counts and fingerprints were read at generation time.
...
The table below is generated in declaration order. Layout separator strings are part of the .set schema and must remain present even though they do not change trading logic. The Source description column is copied verbatim from the .mq5 file in English for direct auditability.
